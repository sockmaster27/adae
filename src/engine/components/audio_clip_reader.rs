...
impl AudioClipReader {
    pub fn new(clip: Arc<AudioClip>, max_buffer_size: usize, sample_rate: u32) -> Self {
    ...
    ///
    /// Each channel in `output` should have the length of `range`.
    fn scale_channels(
        ...
        match input_channels {
            1 => {
                output[0].copy_from_slice(&input[0][input_range.clone()]);
                output[1].copy_from_slice(&input[0][input_range]);
            }
            2 => {
                debug_assert_eq!(input[0].len(), input[1].len());
                output[0].copy_from_slice(&input[0][input_range.clone()]);
                output[1].copy_from_slice(&input[1][input_range]);
            }
            _ => {
        ...
        match &mut self.resampler {
            Some(resampler) => {
                ...
                while filled < output_size {
                    ...
                    let input_range =
                        used..min(used + output_size - filled, resampler.output_frames_max());
