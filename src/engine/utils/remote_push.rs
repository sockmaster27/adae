...
pub trait RemotePushable<E: Send, K: Send>: Send + Debug + Sized {
    ...
    fn new_remote_push_with_capacity(
        ...
        let (event_producer, event_producer_id) = event_queue.add_component();
        (
            RemotePusher {
                length: 0,
            ...
            RemotePushed {
                inner: Self::new_with_capacity(initial_capacity),
                event_producer_id,
            },
        )
    }
...
enum Event<E: Send, K: Send, C: RemotePushable<E, K>> {
    Push(Option<E>),
    PushMultiple(Vec<Option<E>>),
    Remove(Option<K>),
    RemoveMultiple(Vec<Option<K>>),
    Reallocated(C),
}

pub struct RemotePusher<E: Send, K: Send, C: RemotePushable<E, K>> {
...
impl<'a, E: Send, K: Send, C: RemotePushable<E, K>> RemotePusher<E, K, C> {
    ...
    pub fn push_multiple(&mut self, elements: Vec<E>) {
        ...
        self.event_producer
            .send(Event::PushMultiple(option_elements));
    }
    ...
    fn ensure_capacity(&mut self, needed_capacity: usize) {
        if self.capacity < needed_capacity {
            ...
            self.event_producer.send(Event::Reallocated(new_inner));
    ...
    pub fn remove_multiple(&mut self, keys: Vec<K>) {
        ...
        self.event_producer.send(Event::RemoveMultiple(option_keys));
    }
}

pub struct RemotePushed<E: Send, K: Send, C: RemotePushable<E, K>> {
    inner: C,
    event_producer_id: EventProducerId<Event<E, K, C>>,
}
impl<E: Send, K: Send, C: RemotePushable<E, K>> RemotePushed<E, K, C> {
    fn process_event(&mut self, event: &mut Event<E, K, C>) {
        match event {
            Event::Push(e) => self.push(e),
            Event::PushMultiple(es) => {
                for e in es {
                    self.push(e);
                }
            }
            Event::Remove(k) => self.remove(k),
